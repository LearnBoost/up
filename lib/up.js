...
function Worker (server) {
  ...
  var args = [server.file, JSON.stringify(server.requires)];
  this.proc = fork(__dirname + '/worker.js', args, { env: process.env });
...
Worker.prototype.onMessage = function (addr) {
  ...
  if ('spawning' == this.readyState) {
    debug('worker %s listening on port %s', this.pid, addr.port);
    this.port = addr.port;
